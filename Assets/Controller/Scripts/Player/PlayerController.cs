    ...
    public class PlayerController : MonoBehaviour, IPlayerController, IPhysicsObject
    {
        ...
        private void CalculateDash()
        {
            ...
            if (_dashToConsume && _canDash && !Crouching && _time > _nextDashTime)
            {
                ...
                _nextDashTime = _time + PlayerConfigManager.Instance.Config.dashCooldown;
                playerHealth.GrantImmunityFrames(12);
        ...
        private void Move()
        {
            ...
            var targetSpeed = _hasInputThisFrame ? PlayerConfigManager.Instance.Config.baseSpeed : 0;
        ...
        private void SpawnProjectile(float speed, float lifetime, float projectileType, float damage)
        {
            if (AttackProjectile)
            {
        ...
        private void SpawnProjectilesInAllDirections(float speed, float lifetime, float projectileType, int numProjectiles, float damage)
        {
            if (AttackProjectile)
            {
        ...
        private void ShootBeam()
        {
            ...
            if (hit.collider != null)
            {
                ...
                if (hit.collider.gameObject.layer == LayerMask.NameToLayer("Enemy"))
                {
                    ...
                    hit.collider.gameObject.GetComponent<EnemyHealth>().Damage(50);
                }
                else if (hit.collider.gameObject.layer == LayerMask.NameToLayer("Boss"))
                {
        ...
        private IEnumerator PulseEffect()
        {
            ...
            pulseCircle.transform.position = firePoint.position;
            ...
            while (elapsedTime < pulseDuration)
            {
                ...
                foreach (var hitCollider in hitColliders)
                {
                    ...
                    {
                        hitCollider.gameObject.GetComponent<EnemyHealth>().Damage(50);
                    }
                    else if (hitCollider.gameObject.layer == LayerMask.NameToLayer("Boss"))
                    {
                        hitCollider.gameObject.GetComponent<EnemyHealth>().Damage(50);
                    }
                    else if (hitCollider.gameObject.name == "Bolt(Clone)")
                    {
                        Destroy(hitCollider.gameObject);
                    }
                    else if (hitCollider.gameObject.name == "BossBolt(Clone)")
                    {
                        Destroy(hitCollider.gameObject);
                    }
                }
        ...
        //Obselete, replaced with actual sprite - resulted in created sprite being too pixelated.
        ...
        public void Attack()
        {
            ...
            if (_frameInput.AttackDown)
            {
                ...
                switch(config.basicAttackVariant)
                {
                    default:
                        Debug.Log("Default");
                        ...
                        {
                            SpawnProjectile(10.0f, 8.0f, 0.0f,10f);
                    ...
                    case 1:
                        if (Time.time > fireRate)
                        {
                            SpawnProjectile(15.0f, 8.0f, 1.0f,10f);
                            // Set the fireRate to a new time (cooldown period)
                            fireRate = Time.time + 0.5f;
                            Debug.Log("Loop");
                        }
                    ...
                    case 2:
                        if (Time.time > fireRate)
                        {
                            SpawnProjectilesInAllDirections(10.0f, 8.0f, 0.0f, 8, 6f); // 8 projectiles in all directions
                            fireRate = Time.time + 0.5f;
                        }
                        break;
        ...
        public void Ability()
        {
            ...
            {
                switch(config.offensiveAbilityVariant)
                {
                    default:
        ...
        public void Utility()
        {
            ...
            {
                switch(config.utilityAbilityVariant)
                {
                    default:
                    ...
                    case 1:
                        ...
                        break;
                    case 2:
                        playerHealth.GrantImmunityFrames(20);
                        break;
                }
            }
        }
